...
int ts_sock_recvfrom(struct ts_sock *s, void *buf, size_t len,
                     ...
                     int *out_err);
int ts_select(struct ts_sock **read_socks, int read_count,
              struct ts_sock **write_socks, int write_count, int timeout_ms,
              int *out_err, uint8_t *out_read_ready, uint8_t *out_write_ready);
