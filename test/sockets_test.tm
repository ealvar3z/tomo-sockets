...
func main()
    ...
    recv_text := Text.from_utf8(recv_bytes) or fail("tcp receive invalid utf8")
    ...
    when recv_from is Ok(value)
        text := Text.from_utf8(value.data) or fail("udp receive invalid utf8")
        assert text == "pong"
        assert value.addr.port > 0
    is Timeout
        fail("udp receive timeout")
    is Closed
        fail("udp receive closed")
    else
        fail("udp receive error")
