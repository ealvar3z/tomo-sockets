...
struct TlsBuffer(sock:@TlsSocket, data:@[Byte]=@[])
    func new(sock:@TlsSocket -> @TlsBuffer)
        return @TlsBuffer(sock)

    func is_empty(buf:TlsBuffer -> Bool)
        return buf.available() == 0

    func available(buf:TlsBuffer -> Int)
        return buf.data[].length

    func clear(buf:@TlsBuffer)
        buf.data = @[]

    func _append(buf:@TlsBuffer, chunk:[Byte])
        buf.data.insert_all(chunk)

    func _line_from_buffer(buf:@TlsBuffer -> [Byte]?)
        data := buf.data[]
        data_len := Int(data.length)
        if data_len == 0
            return none
    ...
    func receive_n(buf:@TlsBuffer, n:Int, timeout_ms:Int=0 -> SocketRecv)
        if n <= 0
            return SocketRecv.Ok([])
    ...
    func receive_line(buf:@TlsBuffer, timeout_ms:Int=0 -> SocketRecv)
        repeat
            if line := buf._line_from_buffer()
                return SocketRecv.Ok(line)
    ...
    func receive_all(buf:@TlsBuffer, timeout_ms:Int=0 -> SocketRecv)
        repeat
            recv := buf.sock[].receive(max_bytes=8192, timeout_ms=timeout_ms)
            when recv is Ok(data)
                if data.length == 0
